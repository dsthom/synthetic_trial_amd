...
/* 
Script to create & populate the study table: amd_synthetic_lucentis_arm_study_table having run 
...
DROP TABLE IF EXISTS amd_synthetic_lucentis_arm_study_table;

CREATE TABLE amd_synthetic_lucentis_arm_study_table AS
  ...
  WHERE i.InjectedDrugDesc LIKE '%Lucentis%'
  GROUP BY PatientID, EyeCode;
  
CREATE INDEX idx_amd_synthetic_lucentis_arm_study_table_pt_e ON amd_synthetic_lucentis_arm_study_table (`PatientID`, `EyeCode`);

CREATE INDEX idx_amd_synthetic_lucentis_arm_study_table_pt ON amd_synthetic_lucentis_arm_study_table (`PatientID`);

/*
-- extend table with columns to be populated
*/

ALTER TABLE amd_synthetic_lucentis_arm_study_table
  ADD COLUMN patient_eye VARCHAR(255) DEFAULT NULL,
  ADD COLUMN treatment VARCHAR(255) DEFAULT 'lucentis',
  ADD COLUMN gender VARCHAR(255) DEFAULT NULL,
  ADD COLUMN baseline_lucentis_date DATE DEFAULT NULL,
  ...
  ADD COLUMN recent_lucentis_injection DATE DEFAULT NULL,
...
UPDATE amd_synthetic_lucentis_arm_study_table
SET patient_eye = CONCAT(PatientID, '-', EyeCode);

CREATE INDEX idx_amd_synthetic_lucentis_arm_study_table_patient_eye ON amd_synthetic_lucentis_arm_study_table (`patient_eye`);

/*
gender
*/

UPDATE amd_synthetic_lucentis_arm_study_table s
...
/*
-- baseline_lucentis_date (date of first Lucentis injection)
*/

UPDATE amd_synthetic_lucentis_arm_study_table p
SET p.baseline_lucentis_date = (
...
SET baseline_va_date = (
  ...
  WHERE v.PatientID = s.PatientID AND
        ...
        DATEDIFF(s.baseline_lucentis_date, v.EncounterDate) BETWEEN 0 AND 30 AND
        v.EncounterDate <= s.baseline_lucentis_date
);
...
WHERE s.PatientID = v.PatientID AND 
      ...
      s.baseline_lucentis_date = v.EncounterDate
);
...
SET estimated_study_exit = 
  DATE_ADD(baseline_lucentis_date, INTERVAL 378 DAY);
...
SET p.study_exit_54 = (
 ...
 WHERE p.PatientID = v.PatientID AND
       ...
       FLOOR(DATEDIFF(v.EncounterDate, p.baseline_lucentis_date) / 7) BETWEEN 50 AND 58
...
SET study_exit_week_54 = 
  FLOOR(DATEDIFF(study_exit_54, baseline_lucentis_date) / 7);
...
SET p.study_exit_lo = (
  ...
  WHERE p.PatientID = v.PatientID AND 
        p.EyeCode = v.EyeCode AND 
        v.EncounterDate >= p.baseline_lucentis_date AND
...
SET study_exit_week_lo = 
  FLOOR(DATEDIFF(study_exit_lo, baseline_lucentis_date) / 7);
...
/*
--recent_lucentis_injection
*/

UPDATE amd_synthetic_lucentis_arm_study_table s
SET s.recent_lucentis_injection = (
...
UPDATE amd_synthetic_lucentis_arm_study_table
SET drug_recency = 
  DATEDIFF(study_exit, recent_lucentis_injection)
WHERE DATEDIFF(study_exit, recent_lucentis_injection) > 0;
...
UPDATE amd_synthetic_lucentis_arm_study_table p1
	INNER JOIN (
		SELECT PatientID, COUNT(PatientID) AS affected_eyes
		FROM amd_synthetic_lucentis_arm_study_table p2
		GROUP BY PatientID
		) p3
...
UPDATE amd_synthetic_lucentis_arm_study_table p1,
	(SELECT MIN(baseline_lucentis_date) AS index_date, PatientID, EyeCode
	FROM amd_synthetic_lucentis_arm_study_table
	WHERE index_eye IS NULL
	GROUP BY PatientID
	HAVING COUNT(DISTINCT baseline_lucentis_date) > 1) p2
SET index_eye = 1
WHERE p1.PatientID = p2.PatientID AND 
      p1.baseline_lucentis_date = p2.index_date;
...
SELECT * FROM (
    ...
    FROM amd_synthetic_lucentis_arm_study_table s1
    INNER JOIN amd_synthetic_lucentis_arm_study_table s2 ON 
...
UPDATE amd_synthetic_lucentis_arm_study_table p1,
  (SELECT baseline_lucentis_date AS index_date, PatientID
  FROM amd_synthetic_lucentis_arm_study_table
  WHERE index_eye = 1
  ) p2
...
/*
-- age_at_baseline (years between perturbed date of birth and baseline_lucentis_date)
*/
...
SET age_at_baseline = 
  FLOOR(DATEDIFF(p.baseline_lucentis_date, d.PerturbedDateofBirth) / 365.25);
...
/*
-- drug_load (number of Lucentis injections recieved during study period)
*/
...
LEFT JOIN ETCSurgery s
  ...
  WHERE s.ProcedureDesc IN ('stereotactic radiotherapy', 
        ...
        s.EncounterDate <= p.baseline_lucentis_date;
...
LEFT JOIN ETCInjections i
  ...
  WHERE i.InjectedDrugDesc LIKE '%Macugen%' OR
        ...
        i.EncounterDate <= p.baseline_lucentis_date;
...
LEFT JOIN ETCDRGrading d
  ...
  WHERE d.DRGradeDesc LIKE '%PDR%' OR
        ...
        d.EncounterDate <= p.baseline_lucentis_date;
...
ON p.PatientID = s.PatientID AND
  ...
  WHERE s.IndicationDesc LIKE '%cataract%' OR
       ...
       s.EncounterDate >= p.baseline_lucentis_date AND
...
ON p.PatientID = s.PatientID 
  ...
  DATEDIFF(p.baseline_lucentis_date, s.EncounterDate) <= 90;
...
CREATE TEMPORARY TABLE indications_other_than_amd_cataract AS
  ...
  RIGHT JOIN amd_synthetic_lucentis_arm_study_table st
  ON su.PatientID = st.PatientID AND
     su.EyeCode = st.EyeCode
  WHERE su.EncounterDate <= st.baseline_lucentis_date
...
SET previous_vegf_excl = 1
  WHERE avastin_start_date <= baseline_lucentis_date OR
        eylea_start_date <= baseline_lucentis_date;
...
UPDATE amd_synthetic_lucentis_arm_study_table
SET switch_excl = 1 
  WHERE avastin_start_date >= baseline_lucentis_date AND avastin_start_date < study_exit OR
        eylea_start_date >= baseline_lucentis_date AND eylea_start_date < study_exit;
...
WHERE DATEDIFF(third_injection_date, baseline_lucentis_date) > 70;
...
WHERE baseline_lucentis_date >= '2017-12-01';
