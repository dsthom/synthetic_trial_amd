...
DROP TABLE IF EXISTS amd_synthetic_avastin_arm_study_table;

CREATE TABLE amd_synthetic_avastin_arm_study_table AS
...
CREATE INDEX idx_amd_synthetic_avastin_arm_study_table_pt ON amd_synthetic_avastin_arm_study_table (`PatientID`);
...
CREATE INDEX idx_amd_synthetic_avastin_arm_study_table_patient_eye ON amd_synthetic_avastin_arm_study_table (`patient_eye`);
...
SET baseline_va_date = (
  ...
  WHERE v.PatientID = s.PatientID AND
        ...
        DATEDIFF(s.baseline_avastin_date, v.EncounterDate) BETWEEN 0 AND 30 AND
        v.EncounterDate <= s.baseline_avastin_date
);
...
WHERE s.PatientID = v.PatientID AND 
      ...
      s.baseline_avastin_date = v.EncounterDate
);
...
SET p.study_exit_lo = (
  ...
  WHERE p.PatientID = v.PatientID AND 
        p.EyeCode = v.EyeCode AND 
        v.EncounterDate >= p.baseline_avastin_date AND
...
/*
--recent_avastin_injection
*/

UPDATE amd_synthetic_avastin_arm_study_table s
SET s.recent_avastin_injection = (
...
UPDATE amd_synthetic_avastin_arm_study_table p1
	INNER JOIN (
		...
		FROM amd_synthetic_avastin_arm_study_table p2
		GROUP BY PatientID
		) p3
...
UPDATE amd_synthetic_avastin_arm_study_table p1,
	(SELECT MIN(baseline_avastin_date) AS index_date, PatientID, EyeCode
	FROM amd_synthetic_avastin_arm_study_table
	WHERE index_eye IS NULL
	GROUP BY PatientID
	HAVING COUNT(DISTINCT baseline_avastin_date) > 1) p2
SET index_eye = 1
WHERE p1.PatientID = p2.PatientID AND 
      p1.baseline_avastin_date = p2.index_date;
...
SELECT * FROM (
    ...
    FROM amd_synthetic_avastin_arm_study_table s1
    INNER JOIN amd_synthetic_avastin_arm_study_table s2 ON 
...
UPDATE amd_synthetic_avastin_arm_study_table p1,
  (SELECT baseline_avastin_date AS index_date, PatientID
  FROM amd_synthetic_avastin_arm_study_table
  WHERE index_eye = 1
  ) p2
...
/*
-- age_at_baseline (years between perturbed date of birth and baseline_avastin_date)
*/
...
SET age_at_baseline = 
  FLOOR(DATEDIFF(p.baseline_avastin_date, d.PerturbedDateofBirth) / 365.25);
...
LEFT JOIN ETCSurgery s
  ...
  WHERE s.ProcedureDesc IN ('stereotactic radiotherapy', 
        ...
        s.EncounterDate <= p.baseline_avastin_date;
...
LEFT JOIN ETCDRGrading d
  ...
  WHERE d.DRGradeDesc LIKE '%PDR%' OR
        ...
        d.EncounterDate <= p.baseline_avastin_date;
...
ON p.PatientID = s.PatientID AND
  ...
  WHERE s.IndicationDesc LIKE '%cataract%' OR
       ...
       s.EncounterDate >= p.baseline_avastin_date AND
...
ON p.PatientID = s.PatientID 
  ...
  DATEDIFF(p.baseline_avastin_date, s.EncounterDate) <= 90;
...
CREATE TEMPORARY TABLE indications_other_than_amd_cataract AS
  ...
  RIGHT JOIN amd_synthetic_avastin_arm_study_table st
  ON su.PatientID = st.PatientID AND
     su.EyeCode = st.EyeCode
  WHERE su.EncounterDate <= st.baseline_avastin_date
...
WHERE DATEDIFF(third_injection_date, baseline_avastin_date) > 70;
