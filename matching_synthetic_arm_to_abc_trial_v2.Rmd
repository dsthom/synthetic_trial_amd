...
for (i in 1:10){ # state n iterations
  iteration.list[[i]] <- ps %>% 
    # shuffle each iteration (NECESSARY IF INCLUDED IN FUNCTION??)
    sample_n(size = nrow(ps), # n must be equal to n_rows
             replace = FALSE)  
...
```{r}
psm_for_map <- function(.x, match_ratio, caliper_sd, baseline_seed){
   # set seed
...
```{r}
iteration.list.col <- iteration.list.col %>% 
  mutate(psm = map(.x = data, ~ psm_for_map(.x = .x,
                                            match_ratio = 1,
                                            caliper_sd = 0.1,
                                            baseline_seed = 1337)))
```

Error: matches controls with controls at present
